...
| 03 — Authentication and households | Complete locally | CSRF, BCrypt registration, required email confirmation, one-hour single-use password reset, session revocation, opaque session persistence, invitation acceptance, switching, role isolation | Real Better Auth hash evidence, production Google OAuth credentials/callback |
...
| 07 — Shares, email, PDF | Complete locally | Share exchange/cookie isolation, source redaction, encrypted delivery cleanup, Cloudflare HTTPS email delivery with validated provider responses, responsive branded HTML and plain-text transactional emails, valid PDFBox PDF | Provider bounce policy and visual PDF golden approval |
| 08 — Integrations and operations | Complete locally | 68-operation OpenAPI, full isolated 18-page SPA, MCP 2025-11-25 with 17 tools, resumable importer, checksum-guarded no-op migration, performance budgets, layered JAR extraction, and runbooks | Atlas sync, restored-copy rehearsal, load/soak, container and SBOM scans |

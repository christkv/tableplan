...
export default function Favorites({ loaderData }: Route.ComponentProps) {
  return (
    <div className="page-shell"><header className="page-header"><div><p className="eyebrow">Saved recipes</p><h1>Favorites</h1><p className="page-subtitle">Reliable choices for a busy week.</p></div></header>
      {loaderData.recipes.length ? <div className="recipe-grid">{loaderData.recipes.map((recipe) => <Link key={recipe.id} to={`/recipes/${recipe.id}`} className="recipe-card"><div className="recipe-card-top"><h2 title={recipe.name}>{recipe.name}</h2><Heart size={18} fill="currentColor" aria-hidden="true" /></div><div className="recipe-card-body"><p>{recipe.description || recipe.ingredients.slice(0, 4).join(", ")}</p></div><div className="recipe-meta"><span><Users size={15} /> {recipe.servings ?? "?"} servings</span><div className="tag-row">{recipe.tags.slice(0, 2).map((tag) => <Badge key={tag}>{tag}</Badge>)}</div></div></Link>)}</div>

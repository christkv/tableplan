...
import { createSavedRecipeSearch, listSavedRecipeSearches } from "../../src/db/saved-searches";

export async function loader({ request, context }: Route.LoaderArgs) {
  const { env, ctx } = context.get(cloudflareContext);
  const access = await requireApiScope(request, env, ctx, "recipes:read");
  if (access instanceof Response) return access;
  return Response.json({ savedSearches: await listSavedRecipeSearches(env.DB, access.householdId) });
}
...
export async function action({ request, context }: Route.ActionArgs) {
  ...
  try {
    ...
    const savedSearch = await createSavedRecipeSearch(env.DB, {

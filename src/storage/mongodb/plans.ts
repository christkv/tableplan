...
export function createMongoPlanStore(database: Db, recipes: MongoRecipeStore): MongoPlanStore {
  ...
  const ensurePlan = async (input: Parameters<MongoPlanStore["ensure"]>[0]) => {
    ...
    const plan = await plans.findOne({ householdId: input.householdId, startsOn: input.startsOn, endsOn: input.endsOn }, { projection: { _id: 1 } });
  ...
  return {
    ...
    async copyWeek(input) {
      await requireMember(input.userId, input.householdId); const sourceEndsOn = addDays(input.sourceStartsOn, 6); const targetEndsOn = addDays(input.targetStartsOn, 6);
        const source = await plans.findOne({ householdId: input.householdId, startsOn: input.sourceStartsOn, endsOn: sourceEndsOn });
        if (!source?.items.length) throw new MealPlanCopyError("source_empty", "The previous week has no meals to copy");
        const targetId = await ensurePlan({ householdId: input.householdId, userId: input.userId, startsOn: input.targetStartsOn, endsOn: targetEndsOn, timezone: input.timezone });
        const target = await plans.findOne({ _id: targetId }); if (target?.items.length) throw new MealPlanCopyError("target_not_empty", "The target week already contains meals");
        ...
        await plans.updateOne({ _id: targetId, items: { $size: 0 } }, { $set: { items: copied, updatedAt: new Date() } });
